...
class TestRPDBService:
    ...
    def test_get_poster_with_valid_imdb_id(self, rpdb_service):
        """Test getting a poster URL with a valid IMDB ID."""
        poster_url = rpdb_service.get_poster("tt0137523")

        assert poster_url == "https://api.ratingposterdb.com/test-api-key/imdb/poster-default/tt0137523.jpg"

    def test_get_poster_with_imdb_id_without_tt_prefix(self, rpdb_service):
        """Test getting a poster URL with an IMDB ID without 'tt' prefix."""
        poster_url = rpdb_service.get_poster("0137523")

        assert poster_url == "https://api.ratingposterdb.com/test-api-key/imdb/poster-default/tt0137523.jpg"

    def test_get_poster_with_empty_imdb_id(self, rpdb_service):
        """Test getting a poster URL with an empty IMDB ID."""
        poster_url = rpdb_service.get_poster("")

        assert poster_url is None

    def test_get_poster_with_none_imdb_id(self, rpdb_service):
        """Test getting a poster URL with None as IMDB ID."""
        poster_url = rpdb_service.get_poster(None)

        assert poster_url is None

    def test_get_poster_with_empty_api_key(self):
        """Test getting a poster URL with an empty API key."""
        service = RPDBService("")
        poster_url = service.get_poster("tt0137523")

        assert poster_url is None

    def test_get_poster_with_none_api_key(self):
